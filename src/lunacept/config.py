    ...
    *,
    ...
    if max_trace_depth is not None:
        if not isinstance(max_trace_depth, int) or max_trace_depth <= 0:
            raise ValueError(
                f"max_trace_depth must be a positive integer, got {max_trace_depth!r}"
            )
    ...
    if max_value_len is not None:
        if not isinstance(max_value_len, int) or max_value_len <= 0:
            raise ValueError(
                f"max_value_len must be a positive integer, got {max_value_len!r}"
            )
    ...
    if max_value_depth is not None:
        if not isinstance(max_value_depth, int) or max_value_depth < 0:
            raise ValueError(
                f"max_value_depth must be a non-negative integer, got {max_value_depth!r}"
            )
    ...
    if global_install is not None:
        if not isinstance(global_install, bool):
            raise TypeError(
                f"global_install must be a bool, got {type(global_install).__name__}"
            )
